...
function create$1(node, id, self) {
  ...
  function start(elapsed) {
    ...
    for (i in schedules) {
      ...
      if (o.state === RUNNING) {
        o.state = ENDED;
        o.timer.stop();
        o.on.call("interrupt", node, node.__data__, o.index_2, o.group);
        delete schedules[i];
      }
      ...
      // Cancel any pre-empted transitions.
        ...
        o.state = ENDED;
        o.timer.stop();
        o.on.call("cancel", node, node.__data__, o.index_2, o.group);
        delete schedules[i];
      }
    }
    ...
    self.state = STARTING;
    self.on.call("start", node, node.__data__, self.index_2, self.group);
    ...
    for (i = 0, j = -1; i < n; ++i) {
      if (o = self.tween[i].value.call(node, node.__data__, self.index_2, self.group)) {
        tween[++j] = o;
      }
    }
    tween.length = j + 1;
  ...
  function tick(elapsed) {
    ...
    if (self.state === ENDING) {
      self.on.call("end", node, node.__data__, self.index_2, self.group);
      stop();
    }
  }
...
function interrupt(node, name) {
  ...
  for (i in schedules) {
    ...
    schedule$$1.on.call(active ? "interrupt" : "cancel", node, node.__data__, schedule$$1.index_2, schedule$$1.group);
    delete schedules[i];
  }
...
function collide(radius) {
  ...
  function force() {
    ...
    for (var k = 0; k < iterations; ++k) {
      ...
      for (i = 0; i < n; ++i) {
        node = nodes[i];
        ri = radii[node.index_2], ri2 = ri * ri;
        xi = node.x + node.vx;
        yi = node.y + node.vy;
        tree.visit(apply);
      }
  ...
  function prepare(quad) {
    if (quad.data) return quad.r = radii[quad.data.index_2];
  ...
  function initialize() {
    ...
    radii = new Array(n);
    for (i = 0; i < n; ++i) node = nodes[i], radii[node.index_2] = +radius(node, i, nodes);
  }
...
function link(links) {
  ...
  function defaultStrength(link) {
    return 1 / Math.min(count[link.source.index_2], count[link.target.index_2]);
  }
  ...
  function initialize() {
    ...
    for (i = 0, count = new Array(n); i < m; ++i) {
      ...
      count[link.source.index_2] = (count[link.source.index_2] || 0) + 1;
      count[link.target.index_2] = (count[link.target.index_2] || 0) + 1;
    }

    for (i = 0, bias = new Array(m); i < m; ++i) {
      link = links[i], bias[i] = count[link.source.index_2] / (count[link.source.index_2] + count[link.target.index_2]);
    }
...
function manyBody() {
  ...
  function initialize() {
    ...
    for (i = 0; i < n; ++i) node = nodes[i], strengths[node.index_2] = +strength(node, i, nodes);
  }
  ...
  function accumulate(quad) {
    ...
    // For leaf nodes, accumulate forces from coincident quadrants.
      ...
      q.x = q.data.x;
      q.y = q.data.y;
      do strength += strengths[q.data.index_2];
      while (q = q.next);
    }
  ...
  function apply(quad, x1, _, x2) {
    ...
    do if (quad.data !== node) {
      w = strengths[quad.data.index_2] * alpha / l;
      node.vx += x * w;
      node.vy += y * w;
...
function createEdge(left, right, v0, v1) {
  ...
  cells[left.index_2].halfedges.push(index);
  cells[right.index_2].halfedges.push(index);
  return edge;
}
  ...
  triangles: function() {
    ...
    this.cells.forEach(function(cell, i) {
      ...
      while (++j < m) {
        ...
        s1 = e1.left === site ? e1.right : e1.left;
        if (s0 && s1 && i < s0.index_2 && i < s1.index_2 && triangleArea(site, s0, s1) < 0) {
